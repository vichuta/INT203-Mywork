...
const updateMe5 = (e) => console.log('update me 5', e.status, e.statusCode, e.result)
...
<template>
  <div>
    ...
    <div>
      ...
      <!--ปัญหา คือ button ใน BasicButton2 มี 4 parameter 
          แต่ handle fn รับ parameter แค่ 1-2 ตัว (e,n) -->    
    </div>
    <div>
      ...
      <BasicButton3 button-name="Update Me3" @updateMe="updateMe3($event)"/>  <!-- >> test me 3 false 404 FAIL -->
      
      <BasicButton3 button-name="Update Me4" @updateMe="updateMe4($event)"/>  <!-- >> test me 4 {status: false, statusCode: 404, result: 'FAIL'} -->
      <BasicButton3 button-name="Update Me5" @updateMe="updateMe5($event)"/>  <!-- >> test me 5 false 404 FAIL-->
      <BasicButton3 button-name="Update Me6" @updateMe="updateMe6($event,'DONE')"/>  <!-- >> test me 6 {status: false, statusCode: 404, result: 'FAIL'}-->
    <li>Update ME : RED >> false, 404, 'FAIL'</li>
    <li>Update ME : ORANGE >> [false, 404, 'FAIL']</li>
    <li>Update ME : YELLOW >> { status: false, statusCode: 404, result: 'FAIL' }</li>
    </div>
  </div>
</template>
